defmodule MultipleTransactionsTest do
  ...
  test "in one block", wallet do
    ...
    signed_tx2 = create_signed_tx(account1, account2, 90,
    ...
    Pool.get_and_empty_pool()
    signed_tx4 = create_signed_tx(account1, account2, 100,
      Map.get(Chain.chain_state, account1_pub_key, %{nonce: 0}).nonce + 1, 10)
  ...
  test "in multiple blocks", wallet do
    ...
    # account A has 100 tokens, spends 30 (+10 fee) to B, and two times 20 (+10 fee) to C should succeed
    signed_tx5 = create_signed_tx(account1, account2, 30,
      Map.get(Chain.chain_state, account1_pub_key, %{nonce: 0}).nonce + 1, 10)
    ...
    Pool.get_and_empty_pool()
    signed_tx6 = create_signed_tx(account1, account3, 20,
      Map.get(Chain.chain_state, account1_pub_key, %{nonce: 0}).nonce + 1, 10)
    ...
    Pool.get_and_empty_pool()
    signed_tx7 = create_signed_tx(account1, account3, 20,
      Map.get(Chain.chain_state, account1_pub_key, %{nonce: 0}).nonce + 1, 10)
    ...
    Pool.get_and_empty_pool()
    signed_tx9 = create_signed_tx(account1, account2, 40,
      Map.get(Chain.chain_state, account1_pub_key, %{nonce: 0}).nonce + 1, 10)
    ...
    Pool.get_and_empty_pool()
    signed_tx10 = create_signed_tx(account1, account3, 20,
      Map.get(Chain.chain_state, account1_pub_key, %{nonce: 0}).nonce + 1, 10)
    ...
    Pool.get_and_empty_pool()
    signed_tx11 = create_signed_tx(account1, account3, 20,
      Map.get(Chain.chain_state, account1_pub_key, %{nonce: 0}).nonce + 1, 10)
    ...
    Pool.get_and_empty_pool()
    signed_tx13 = create_signed_tx(account1, account2, 90,
      Map.get(Chain.chain_state, account1_pub_key, %{nonce: 0}).nonce + 1, 10)
    ...
    Pool.get_and_empty_pool()
    signed_tx14 = create_signed_tx(account2, account3, 90,
      Map.get(Chain.chain_state, account2_pub_key, %{nonce: 0}).nonce + 1, 10)
  ...
  test "in one block, miner collects all the fees from the transactions", wallet do
    ...
    Pool.get_and_empty_pool()
    signed_tx3 = create_signed_tx(account1, account3, 90,
      Map.get(Chain.chain_state, account1_pub_key, %{nonce: 0}).nonce + 1, 10)

    assert :ok = Pool.add_transaction(signed_tx3)
    signed_tx4 = create_signed_tx(account2, account3, 90,
      Map.get(Chain.chain_state, account2_pub_key, %{nonce: 0}).nonce + 1, 10)
  ...
  test "locked amount", wallet do
    ...
    :ok = Miner.mine_sync_block_to_chain
    signed_tx2 = create_signed_tx(account1, {from_acc, priv_key}, 50,
      Map.get(Chain.chain_state, account1_pub_key, %{nonce: 0}).nonce + 1, 10)
