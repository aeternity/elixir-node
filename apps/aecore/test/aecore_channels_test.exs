defmodule AecoreChannelTest do
  ...
  test "create channel, treansfer funds, mutal close channel", ctx do
    ...
    {:ok, signed_state} = call_s2({:recv_state, state, ctx.sk2})
    assert :open == get_fsm_state_s2(id)
    {:ok, nil} = call_s1({:recv_state, signed_state, ctx.sk1})
    assert :open == get_fsm_state_s1(id)

    assert 100 == ChannelStateOffChain.initiator_amount(signed_state)
    assert 200 == ChannelStateOffChain.responder_amount(signed_state)
    assert 1 == ChannelStateOffChain.sequence(signed_state)

    {:ok, state} = call_s2({:transfer, id, 170, ctx.sk2})
    {:ok, signed_state} = call_s1({:recv_state, state, ctx.sk1})
    {:ok, nil} = call_s2({:recv_state, signed_state, ctx.sk2})

    assert 270 == ChannelStateOffChain.initiator_amount(signed_state)
    assert 30 == ChannelStateOffChain.responder_amount(signed_state)
    assert 2 == ChannelStateOffChain.sequence(signed_state)
  ...
  test "create channel, transfer twice, slash with old, slash with corrent and settle", ctx do
    ...
    {:ok, state} = call_s1({:transfer, id, 50, ctx.sk1})
    assert :update == get_fsm_state_s1(id)
    {:ok, signed_state1} = call_s2({:recv_state, state, ctx.sk2})
    assert :open == get_fsm_state_s2(id)
    ...
    {:ok, state} = call_s2({:transfer, id, 170, ctx.sk2})
    {:ok, signed_state2} = call_s1({:recv_state, state, ctx.sk1})
