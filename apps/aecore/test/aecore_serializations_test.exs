defmodule AecoreSerializationTest do
  ...
  def create_data(data_type, :elixir) do
    ...
    case data_type do
      ...
      NamePreClaimTx ->
        Account.pre_claim("test.aet", 123, 50) |> Map.get(:data)

      NameClaimTx ->
        Account.claim("test.aet", 123, 50) |> Map.get(:data)

      NameUpdateTx ->
        Account.name_update("test.aet", "{\"test\": 2}", 50, 5000, 50) |> Map.get(:data)

      NameTransferTx ->
        transfer_to_pub = acc2_pub

        Account.name_transfer("test.aet", transfer_to_pub, 50) |> Map.get(:data)
