defmodule Aecore.Keys.Worker do
  ...
  def start_link() do
	GenServer.start_link(__MODULE__, %{pub:        nil,
									   priv:       nil,
									   pass:       nil,
									   keys_dir:   nil,
									   pub_file:   nil,
  ...
  def handle_call({:verify, {term, signature, pub_key}}, _from, %{algo: algo,
																  digest: digest,
																  curve: curve}=state) do
	result = :crypto.verify(algo,
	         digest,
	         :erlang.term_to_binary(term),
	         signature,
	         [pub_key, :crypto.ec_curve(curve)])
    {:reply, result, state}
  end
  ...
  def handle_call({:sign, term}, _from, %{priv:   priv_key,
	...
	{:reply, {:ok, %{data: term, signature: signature}}, state}
  end
  ...
  def handle_call(:delete, _from, %{pub_file:  pub_file,
									priv_file: priv_file}=state) do
	try do
	  :ok = :file.delete(pub_file)
	  :ok = :file.delete(priv_file)
	  {:reply, :ok, %{state
					  | pub_file: nil, priv_file: nil, pub: nil, priv: nil}}
	catch
	  _ ->
		{:reply, :error, state}
	end
  ...
  def handle_call({:new, password}, _from, %{type:       key_type,
											 keys_dir:   keys_dir,
											 curve:      curve}=state) do
	try do
	  {new_pub_file, new_priv_file} = p_gen_filename(keys_dir)
	  {new_pub_key, new_priv_key}   = p_gen_new(password,
		                                        key_type,
		                                        curve,
		                                        new_pub_file,
		                                        new_priv_file)
  ...
  def handle_call({:open, password}, _from, %{pub_file:  pub_file,
											  priv_file: priv_file}=state) do
	try do
  ...
  def handle_call({:set, password, priv, pub}, _from, %{pub_file:  pub_file,
														priv_file: priv_file}=state) do
	try do
  ...
  def handle_info(:timeout, %{pub:      nil,
							  priv:     nil,
							  pass:     password,
							  type:     key_type,
							  curve:    curve,
							  keys_dir: keys_dir}=state) do
	try do
  ...
  defp p_gen_new(password, key_type, curve, pub_filename, priv_filename) do
	{new_pub_key, new_priv_key} =
	  :crypto.generate_key(key_type, :crypto.ec_curve(curve))
