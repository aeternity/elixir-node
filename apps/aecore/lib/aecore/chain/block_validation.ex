defmodule Aecore.Chain.BlockValidation do
  ...
  def validate_block_transactions(block) do
    block.txs
    |> Enum.map(fn
      %SignedTx{data: %VotingTx{voting_payload: data}} ->
        VotingValidation.validate(%SignedTx{data: %VotingTx{voting_payload: data}})
  ...
  @spec filter_invalid_transactions_chainstate(list(SignedTx.t()), map(), integer()) ::
          list(SignedTx.t())
  def filter_invalid_transactions_chainstate(txs_list, chain_state, block_height) do
    {valid_txs_list, _} =
      List.foldl(txs_list, {[], chain_state}, fn
        %SignedTx{data: %VotingTx{}} = tx, {valid_txs_list, chain_state_acc} ->
