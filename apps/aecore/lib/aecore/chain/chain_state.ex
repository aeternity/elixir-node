defmodule Aecore.Chain.ChainState do
  ...
  def apply_tx!(transaction, chain_state, block_height) do
    if SignedTx.is_coinbase?(transaction) do
      apply_fun_on_map(chain_state, transaction.data.to_acc, fn a ->
        Account.tx_in!(a, transaction.data, block_height)
      end)
    else
      ...
      chain_state =
        apply_fun_on_map(chain_state, transaction.data.from_acc, fn a ->
          Account.tx_out!(a, transaction.data, block_height)
        end)
