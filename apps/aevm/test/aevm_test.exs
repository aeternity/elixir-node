defmodule AevmTest do
  ...
  test_with_params "vmArithmeticTest1",
    fn config_name ->
      # TODO
      IO.inspect(load_test_config(:vmArithmeticTest, config_name))
    end do
      [
        {:add0},
        {:add1},
  ...
  test_with_params "vmBitwiseLogicOperation1",
    fn (config_name) ->
      # TODO:
      IO.inspect(load_test_config(:vmBitwiseLogicOperation, config_name))
    end do
      [
        {:and0},
        {:and1},
  ...
  test_with_params "vmBlockInfoTest1",
    fn (config_name) ->
      # TODO:
      IO.inspect(load_test_config(:vmBlockInfoTest, config_name))
    end do
      [
        {:blockhash257Block},
        {:blockhash258Block},
  ...
  test_with_params "vmEnvironmentalInfo1",
    fn (config_name) ->
      # TODO:
      IO.inspect(load_test_config(:vmEnvironmentalInfo, config_name))
    end do
      [
  ...
  test_with_params "vmIOandFlowOperations1",
    fn (config_name) ->
      # TODO:
      IO.inspect(load_test_config(:vmIOandFlowOperations, config_name))
    end do
      [
  ...
  test_with_params "vmLogTest1",
    fn (config_name) ->
      # TODO:
      IO.inspect(load_test_config(:vmLogTest, config_name))
    end do
      [
        {:log0_emptyMem},
  ...
  test_with_params "vmPerformance1",
    fn (config_name) ->
      # TODO:
      IO.inspect(load_test_config(:vmPerformance, config_name))
    end do
      [
        {:ackermann31},
        {:ackermann32},
  ...
  test_with_params "vmPushDupSwapTest1",
    fn (config_name) ->
      # TODO:
      IO.inspect(load_test_config(:vmPushDupSwapTest, config_name))
    end do
      [
      {:dup1},
      {:dup10},
  ...
  test_with_params "vmRandomTest1",
    fn (config_name) ->
      # TODO:
      IO.inspect(load_test_config(:vmRandomTest, config_name))
    end do
      [
        {:"201503102037PYTHON"},
        {:"201503102148PYTHON"},
  ...
  test_with_params "vmSha3Test1",
    fn (config_name) ->
      # TODO:
      IO.inspect(load_test_config(:vmSha3Test, config_name))
    end do
      [
        {:sha3_0},
        {:sha3_1},
  ...
  test_with_params "vmSystemOperations1",
    fn (config_name) ->
      # TODO:
      IO.inspect(load_test_config(:vmSystemOperations, config_name))
    end do
      [
        {:ABAcalls0},
        {:ABAcalls1},
  ...
  test_with_params "vmTests1",
    fn (config_name) ->
      # TODO:
      IO.inspect(load_test_config(:vmTests, config_name))
    end do
      [
        {:arith},
        {:boolean},
  ...
  defp load_test_config(config_folder_atom, config_name_atom) do
    ...
    config = File.read!("../../aevm_external/ethereum_tests/VMTests/#{config_folder_string}/#{config_name_string}.json")
    json_map = Poison.decode!(config)
    parse_config_value(Map.to_list(json_map), Map.to_list(config_structure()), %{})
  end
  ...
  defp parse_config_value(
         [{c_key, c_value} | _] = _config,
         [{:multiple_atom, s_value} | _] = _structure,
         result
       ) do
    Map.put(
      result,
      String.to_atom(c_key),
      parse_config_value(Map.to_list(c_value), Map.to_list(s_value), %{})
    )
  end

  defp parse_config_value(
         [{c_key, c_value}] = _config,
         [{:multiple_bin_int, :data_array_int}] = _structure,
         result
       ) do
    <<"0x", hex_bin::binary>> = c_key
  ...
  defp parse_config_value(
         [] = _config,
         [{:multiple_bin_int, _} | _] = _structure,
         result
       ) do
    result
  end
  ...
  defp parse_config_value([{c_key, c_value} | c_rest], [{_, :string} | s_rest], result)
       when is_bitstring(c_value) do
    parse_config_value(c_rest, s_rest, Map.put(result, String.to_atom(c_key), c_value))
  end

  defp parse_config_value([{c_key, c_value} | c_rest], [{_, :bin_int} | s_rest], result)
       when is_bitstring(c_value) do
    ...
    parse_config_value(
      c_rest,
      s_rest,
      Map.put(result, String.to_atom(c_key), new_value)
    )
  end

  defp parse_config_value([{c_key, c_value} | c_rest], [{_, :data_hex} | s_rest], result)
       when is_bitstring(c_value) do
    <<"0x", bytecode::binary>> = c_value
    parse_config_value(c_rest, s_rest, Map.put(result, String.to_atom(c_key), bytecode))
  end

  defp parse_config_value([{c_key, c_value} | c_rest], [{_, :data_array} | s_rest], result)
       when is_bitstring(c_value) do
    parse_config_value(
      c_rest,
      s_rest,
      Map.put(result, String.to_atom(c_key), State.bytecode_to_bin(c_value))
    )
  end

  defp parse_config_value([{c_key, c_value} | c_rest], [{_, :unclear} | s_rest], result)
       when is_bitstring(c_value) do
    parse_config_value(c_rest, s_rest, Map.put(result, String.to_atom(c_key), c_value))
  end

  defp parse_config_value([], [_ | _], _) do
    []
  end

  defp parse_config_value([{c_key, c_value} | c_rest] = a, [{_, s_value} | s_rest] = b, result) do
    new_c_value =
      if is_map(c_value) do
        Map.to_list(c_value)
      else
        if !Enum.empty?(c_value) do
          c_value
          |> Enum.at(0)
          |> Map.to_list()
        else
    ...
    new_c_key = String.to_atom(c_key)

    if new_c_key == :callcreates do
      callcreate = parse_config_value(new_c_value, new_s_value, %{})
      callcreates = Map.get(result, :callcreates, [])
      parse_config_value(c_rest, s_rest, Map.put(result, :callcreates, callcreates ++ [callcreate]))
    else
      new_result =
        Map.put(
          result,
          String.to_atom(c_key),
          parse_config_value(new_c_value, new_s_value, %{})
        )

      parse_config_value(c_rest, s_rest, new_result)
    end
  end
  ...
  defp get_config_names_atoms(folder) do
    Path.wildcard("../../aevm_external/ethereum_tests/VMTests/" <> folder <> "/*.json")
